...
`python3 -m src.post_all` を実行。全プロファイルの scored_papers を読んでチャンネルに配信。1プロファイルがエラーでも残りは続行する。

### ステップ4: INSPIRE プロファイル更新

配信した論文の著者に profiles/ 登録者（`inspire_bai` 設定あり）がいた場合、以下を実行:

```
python3 -c "from src.profile_update import check_for_profile_updates; import json; papers=json.load(open('state/today_papers_odakin.json'))['papers']; check_for_profile_updates(papers)"
```
